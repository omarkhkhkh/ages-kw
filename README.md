...
## التشغيل بـ Docker (الأسهل — أمر واحد)
...
يشغّل هذا الأمر ثلاث حاويات: قاعدة البيانات PostgreSQL، سيرفر الـAPI (يطبّق مخطط قاعدة البيانات ويزرع حساب المدير تلقائيًا عند الإقلاع)، وواجهة الموقع خلف nginx. الملفات المرفوعة وبيانات قاعدة البيانات محفوظة في Docker volumes فلا تضيع عند إعادة التشغيل.

| أمر | الوظيفة |
|---|---|
| `docker compose up -d --build` | تشغيل/إعادة بناء النظام كاملًا |
| `docker compose logs -f api` | متابعة سجلات السيرفر |
| `docker compose down` | إيقاف النظام (البيانات تبقى) |
| `docker compose down -v` | إيقاف **مع حذف** كل البيانات ⚠️ |

> ⚠️ قبل أي نشر حقيقي: غيّر `SESSION_SECRET` وكلمة سر قاعدة البيانات في `docker-compose.yml`، وغيّر كلمة سر `admin` بعد أول دخول.

## التشغيل يدويًا (للتطوير)
