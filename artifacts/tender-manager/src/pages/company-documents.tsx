...
const PREDEFINED = [
  "الرخصة التجارية", "السجل التجاري", "شهادة غرفة التجارة",
  "اعتماد التوقيع", "الوكالات التجارية", "شهادة الزكاة والضريبة",
  "شهادة الجودة ISO", "عقد التأسيس", "النظام الأساسي", "أخرى",
];
...
export default function CompanyDocuments() {
  ...
  const { data: stats } = useQuery<any>({
    queryKey: ["company-docs-stats"],
    queryFn: () => apiFetch("/api/company-documents/stats"),
    refetchInterval: 60000,
  });

  const { data: rows = [], isLoading } = useQuery<any[]>({
    queryKey: ["company-docs", search],
    queryFn: () => apiFetch("/api/company-documents" + (search ? `?search=${encodeURIComponent(search)}` : "")),
  });
  ...
  const upsert = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["company-docs"] }); qc.invalidateQueries({ queryKey: ["company-docs-stats"] }); closeForm(); },
  });

  const del = useMutation({
    mutationFn: (id: number) => apiFetch(`/api/company-documents/${id}`, { method: "DELETE" }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["company-docs"] }); qc.invalidateQueries({ queryKey: ["company-docs-stats"] }); },
  });
